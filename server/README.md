# MAX Efficiency - Backend Server

Backend сервер для приложения MAX Efficiency, построенный на Node.js, Express и PostgreSQL.

## 🎯 Описание

Это REST API сервер, который обеспечивает:

- Аутентификацию пользователей через MAX платформу
- Управление задачами (CRUD операции)
- Бизнес-метрики и статистику
- Систему челленджей
- Настройки пользователя

## 🛠 Технологии

- **Node.js** 20.x - Runtime окружение
- **Express** 4.x - Web framework
- **PostgreSQL** 16.x - База данных
- **TypeScript** 5.x - Типизация
- **JWT** - Аутентификация
- **Helmet** - Безопасность
- **CORS** - Cross-Origin Resource Sharing

## 📦 Установка

### Через Docker (рекомендуется)

```bash
# Из корневой директории проекта
docker-compose up backend
```

### Локальная установка

```bash
# Установка зависимостей
npm install

# Настройка переменных окружения
cp .env.example .env
# Отредактируйте .env файл

# Запуск миграций
npm run migrate:dev

# Запуск в режиме разработки
npm run dev

# Сборка для production
npm run build

# Запуск production версии
npm start
```

## ⚙️ Конфигурация

### Переменные окружения (.env)

```env
# База данных
DATABASE_URL=postgresql://user:password@localhost:5432/maxflow

# Сервер
PORT=3000
NODE_ENV=development

# JWT
JWT_SECRET=your-secret-key-change-in-production

# CORS
CORS_ORIGIN=http://localhost:5173
```

## 🗄 База данных

### Миграции

```bash
# Запуск миграций (development)
npm run migrate:dev

# Запуск миграций (production)
npm run migrate
```

### Структура таблиц

#### users
- `id` - UUID, первичный ключ
- `max_user_id` - ID пользователя в MAX
- `phone` - Телефон пользователя
- `created_at` - Дата создания
- `updated_at` - Дата обновления

#### tasks
- `id` - UUID, первичный ключ
- `user_id` - Внешний ключ на users
- `title` - Название задачи
- `description` - Описание
- `status` - Статус (pending, in_progress, completed)
- `priority` - Приоритет (low, medium, high)
- `due_date` - Дедлайн
- `completed_at` - Дата завершения
- `created_at` - Дата создания
- `updated_at` - Дата обновления

#### challenges
- `id` - UUID, первичный ключ
- `user_id` - Внешний ключ на users
- `type` - Тип челленджа
- `target` - Целевое значение
- `current` - Текущее значение
- `status` - Статус
- `created_at` - Дата создания
- `updated_at` - Дата обновления

## 🔌 API Endpoints

### Аутентификация

#### POST /api/auth/max
Аутентификация через MAX платформу

**Request:**
```json
{
  "initData": "string"
}
```

**Response:**
```json
{
  "token": "jwt_token",
  "user": {
    "id": "uuid",
    "maxUserId": "string",
    "phone": "string"
  }
}
```

### Задачи

#### GET /api/tasks
Получить список задач пользователя

**Query params:**
- `status` - Фильтр по статусу
- `priority` - Фильтр по приоритету
- `date` - Фильтр по дате

**Response:**
```json
[
  {
    "id": "uuid",
    "title": "string",
    "description": "string",
    "status": "pending",
    "priority": "high",
    "dueDate": "2024-01-01",
    "completedAt": null,
    "createdAt": "2024-01-01T00:00:00Z"
  }
]
```

#### POST /api/tasks
Создать новую задачу

**Request:**
```json
{
  "title": "string",
  "description": "string",
  "priority": "high",
  "dueDate": "2024-01-01"
}
```

#### PUT /api/tasks/:id
Обновить задачу

**Request:**
```json
{
  "title": "string",
  "status": "completed",
  "priority": "medium"
}
```

#### DELETE /api/tasks/:id
Удалить задачу

### Бизнес-метрики

#### GET /api/business/stats
Получить статистику

**Response:**
```json
{
  "totalTasks": 100,
  "completedTasks": 75,
  "completionRate": 0.75,
  "averageCompletionTime": 3600000
}
```

### Челленджи

#### GET /api/challenges
Получить список челленджей

#### POST /api/challenges
Создать новый челлендж

#### PUT /api/challenges/:id
Обновить челлендж

### Настройки

#### GET /api/settings
Получить настройки пользователя

#### PUT /api/settings
Обновить настройки

## 🔒 Безопасность

- **Helmet** - Защита HTTP заголовков
- **CORS** - Контроль доступа
- **JWT** - Токены аутентификации
- **Валидация** - Проверка входных данных
- **Rate limiting** - Ограничение запросов (рекомендуется добавить)

## 🧪 Разработка

### Структура проекта

```
server/
├── src/
│   ├── config/           # Конфигурация
│   │   ├── database.ts   # Настройки БД
│   │   └── env.ts        # Переменные окружения
│   ├── controllers/      # Контроллеры
│   │   ├── authController.ts
│   │   ├── tasksController.ts
│   │   ├── businessController.ts
│   │   ├── challengesController.ts
│   │   └── settingsController.ts
│   ├── middleware/       # Middleware
│   │   └── auth.ts       # Проверка JWT
│   ├── routes/           # Маршруты
│   │   ├── auth.ts
│   │   ├── tasks.ts
│   │   ├── business.ts
│   │   ├── challenges.ts
│   │   └── settings.ts
│   ├── migrations/       # Миграции БД
│   │   ├── 001_create_users_table.ts
│   │   ├── 002_create_app_tables.ts
│   │   ├── 003_add_completed_at_to_tasks.ts
│   │   ├── 004_convert_task_ids_to_uuid.ts
│   │   └── run.ts
│   ├── types/            # TypeScript типы
│   │   └── index.ts
│   ├── utils/            # Утилиты
│   │   ├── jwt.ts
│   │   └── maxAuth.ts
│   └── index.ts          # Точка входа
├── package.json
├── tsconfig.json
└── README.md
```

### Команды разработки

```bash
# Разработка с hot-reload
npm run dev

# Сборка
npm run build

# Запуск production
npm start

# Миграции (dev)
npm run migrate:dev

# Миграции (prod)
npm run migrate
```

## 🐛 Отладка

### Логирование

Сервер использует console.log для логирования. В production рекомендуется использовать winston или pino.

### Проверка подключения к БД

```bash
# Через Docker
docker-compose exec postgres psql -U maxflow -d maxflow

# Локально
psql -U maxflow -d maxflow
```

### Проверка миграций

```sql
-- Проверить примененные миграции
SELECT * FROM migrations;

-- Проверить структуру таблиц
\dt
\d users
\d tasks
```

## 📝 Лицензия

MIT